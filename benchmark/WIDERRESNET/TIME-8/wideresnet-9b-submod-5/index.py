...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/wideresnet-9b/TIME_OAP/8/wideresnet-9b-submod-5.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/wideresnet-9b/TIME_OAP/8/wideresnet-9b-inputs-5.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/wideresnet-9b-submod-6-time-8.cdgp#"+str(uid), timeout=timeout) as response:
