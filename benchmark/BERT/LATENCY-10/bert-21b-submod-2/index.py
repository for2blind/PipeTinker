...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/bert-21b/LATENCY_OAP/10.0-10/bert-21b-submod-2.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/bert-21b/LATENCY_OAP/10.0-10/bert-21b-inputs-2.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/bert-21b-submod-3-latency-10.cdgp#"+str(uid), timeout=timeout) as response:
