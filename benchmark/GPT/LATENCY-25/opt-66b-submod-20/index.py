...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/opt-66b/LATENCY_OAP/10.0-25/opt-66b-submod-20.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/opt-66b/LATENCY_OAP/10.0-25/opt-66b-inputs-20.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/opt-66b-submod-21-latency-25.cdgp#"+str(uid), timeout=timeout) as response:
