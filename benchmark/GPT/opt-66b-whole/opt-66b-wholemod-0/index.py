...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/wholegraphs/opt-66b//opt-66b-wholemod-0.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/wholegraphs/opt-66b/opt-66b-wholemod-inputs-0.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/opt-66b-wholemod-1-opt-66b-whole.cdgp#"+str(uid), timeout=timeout) as response:
