...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/whisper-large-v2/PARAM_OAP/3/whisper-large-v2-submod-0.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/whisper-large-v2/PARAM_OAP/3/whisper-large-v2-inputs-0.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/whisper-large-v2-submod-1-param-3.cdgp#"+str(uid), timeout=timeout) as response:
